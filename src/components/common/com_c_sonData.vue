<template>
  <div>
    ...
    <el-dialog
      ...
      v-if="isShowDialog"
    >
      <com_page_detail_group :prop_groupId="doc[this.cf.column]" @update_count="update_count"></com_page_detail_group>
...
export default {
  ...
  computed: {
    title() {
      ...
      return `【${this.doc[key]}】的子${this.cf.dataTypeName}`
    }
  },
  methods: {
    ...
    addSonDataG: async function () {
      ...
      //变量：{父Id}
      let idParentData = this.doc._idRel2 || this.doc._id//有可能是分组数据，也有可能是原始列表数据
      let dataAdd = { title: `${dataTypeNameParent}[${idParentData}]的子${this.cf.dataTypeName}列表`, "dataType": this.cf.dataType }

      let { data: { addData } } = await axios({//ajax添加一个分组
        method: "post", url: `${PUB.domain}/info/commonAdd`,
        data: { tips:`ajax添加一个分组`,"_data": dataAdd, "_systemId": "sys_api", "_dataType": "group" }
      });
